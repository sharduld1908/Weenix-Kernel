...
sys_read(read_args_t *arg)
{
        ...
        copy_from_user(&kern_args, arg, sizeof(kern_args)); //error handling not tested

        temp_buf = page_alloc();

        if((num_bytes = do_read(kern_args.fd, temp_buf, kern_args.nbytes)) < 0){
                curthr->kt_errno = -num_bytes;
                page_free(temp_buf);
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                return -1;
        }

        copy_to_user(arg->buf, temp_buf, num_bytes); //error handling not tested
...
sys_write(write_args_t *arg)
{
        ...
        temp_buf = page_alloc();
        copy_from_user(temp_buf, kern_args.buf, arg->nbytes); //error handling not tested

        if((num_bytes = do_write(kern_args.fd, temp_buf, kern_args.nbytes)) < 0){
                curthr->kt_errno = -num_bytes;
                page_free(temp_buf);
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                return -1;
...
sys_getdents(getdents_args_t *arg)
{
        ...
        for (size_t i = 0; i < kern_args.count / sizeof(dirent_t); i++)
        {
                num_bytes = do_getdent(kern_args.fd, &dirp);
                if (num_bytes < 0)
                {
                        curthr->kt_errno = -num_bytes;
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");
                ...
                uint32_t k_addr = (uint32_t)kern_args.dirp + total_num_bytes;
                copy_to_user((void *)k_addr, &dirp, sizeof(dirent_t)); //error handling not tested
